...
export default function AccountPage() {
  ...
  async function loadAccount() {
    ...
    if (authError || !authData.user) {
      setMessage(t('account.mustBeLoggedIn'))
      setLoading(false)
      return
    }
  ...
  async function saveProfileInfo() {
    ...
    if (!cleanedUsername) {
      setMessage(t('account.usernameRequired'))
      return
    }

    if (!cleanedDisplayName) {
      setMessage(t('account.displayNameRequired'))
      return
    }
    ...
    if (existingUser) {
      setMessage(t('account.usernameTaken'))
      return
    }
  ...
  async function uploadAvatar(file?: File) {
    ...
    if (!file.type.startsWith('image/')) {
      setMessage(t('account.onlyImagesAllowed'))
      setUploadingAvatar(false)
      return
    }

    if (file.size > 2 * 1024 * 1024) {
      setMessage(t('account.avatarTooLarge'))
      setUploadingAvatar(false)
      return
    }
  ...
  async function changeEmail() {
    ...
    if (!newEmail.trim()) {
      setMessage(t('account.emailRequired'))
      return
    }
    ...
    setMessage(t('account.emailUpdateRequested'))
  }
  ...
  if (loading) {
    return <div style={{ padding: 40 }}>{t('account.loading')}</div>
  }
  ...
  if (!user) {
    return (
      <div style={{ padding: 40, maxWidth: '700px', margin: '0 auto' }}>
        ...
        <Link href={`/${locale}/login`}>{t('account.goToLogin')}</Link>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: 40, maxWidth: '700px', margin: '0 auto' }}>
      <Link href={`/${locale}/profile`} style={{ display: 'block', marginBottom: '20px' }}>
        ← {t('account.backToProfile')}
      </Link>

      <h1 style={{ marginBottom: '24px' }}>{t('account.title')}</h1>

      <div style={{ marginBottom: '36px' }}>
        <h2 style={{ marginBottom: '12px' }}>{t('account.profileInfo')}</h2>
      ...
      <div style={{ marginBottom: '36px' }}>
        <h2 style={{ marginBottom: '12px' }}>{t('account.avatar')}</h2>
        ...
        {uploadingAvatar && <p>{t('account.uploadingAvatar')}</p>}
      ...
      <div style={{ marginBottom: '36px' }}>
        <h2 style={{ marginBottom: '12px' }}>{t('account.email')}</h2>
      ...
      <div style={{ marginBottom: '36px' }}>
        <h2 style={{ marginBottom: '12px' }}>{t('account.password')}</h2>
