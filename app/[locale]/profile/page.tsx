  ...
  if (!user) {
    return (
      ...
      >
        ...
        <Link href={`/${locale}/login`}>{t('profile.login')}</Link>
      </div>
    )
  }

  const displayName = profile?.display_name || profile?.username || t('nav.profile')
  const usernameText = profile?.username ? `@${profile.username}` : t('profile.noUsernameYet')
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '14px' }}>
        <h2 style={{ margin: 0 }}>{t('profile.myRecipes')}</h2>
      </div>

      {recipes.length === 0 ? (
        <p style={{ color: '#aaa' }}>{t('profile.noRecipesYet')}</p>
      ) : (
        <div
          style={{
            display: 'grid',
              ...
              >
                ...
                >
                  ...
                  <div style={{ minWidth: 0, flex: 1, paddingRight: '56px' }}>
                    ...
                    >
                      <div style={{ marginBottom: '4px' }}>{getTranslatedCategory(recipe.Category)}</div>
                      <div>
                        {t('home.prep')} {recipe.Prep_time ?? '-'} {t('home.min')}
                      </div>
                      <div>
                        {t('home.cook')} {recipe.Cook_time ?? '-'} {t('home.min')}
                      </div>
                    </div>
                  </div>
