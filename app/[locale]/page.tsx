...
function HomeContent() {
  ...
  const sortOptions = [
    ...
    { value: 'Prep time', label: t('sort.prepTime') },
    { value: 'Cook time', label: t('sort.cookTime') },
    { value: 'Favorites first', label: t('sort.favoritesFirst') },
  ]
  ...
  async function toggleFavorite(recipeId: number) {
    ...
    if (!user) {
      setMessage(t('messages.mustBeLoggedInToFavorite'))
      return
    }
    ...
    >
      ...
      >
        ...
        {!isMobile && (
          ...
          >
            {showOnlyFavorites
              ? t('home.showingFavorites')
              : t('home.showFavorites')}
          </button>
        )}
      </div>
...
export default function Home() {
  ...
  return (
    <Suspense fallback={<div style={{ padding: 40, color: 'white' }}>{t('home.loading')}</div>}>
      <HomeContent />
    </Suspense>
  )
}