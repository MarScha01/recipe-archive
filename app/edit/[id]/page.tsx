...
export default function EditRecipePage() {
  ...
  async function loadRecipe() {
    ...
    if (!currentUser) {
      setMessage('You must be logged in to edit recipes.')
      setAuthorized(false)
    ...
    if (!isOwner && !isAdmin) {
      setMessage('You are not allowed to edit this recipe.')
      setAuthorized(false)
      setCheckingAccess(false)
      return
    }
  ...
  async function deleteRecipe() {
    if (!authorized) {
      setMessage('You are not allowed to delete this recipe.')
      return
    }
  ...
  if (!authorized) {
    return (
      <div style={{ padding: 40, maxWidth: '900px', margin: '0 auto' }}>
        ...
        <p>{message || 'You are not allowed to view this page.'}</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: 40, maxWidth: '900px', margin: '0 auto' }}>
      <Link href={`/recipe/${id}`} style={{ display: 'block', marginBottom: '20px' }}>
        ← Back to recipe
      </Link>
